...
%token STRUCT IF ELSE WHILE RETURN DOT SEMI COMMA ASSIGN LT LE GT GE NE EQ PLUS MINUS MUL DIV AND OR NOT LP RP LB RB LC RC LEXERR FOR
...
Stmt 
    ...
    | FOR LP ForExps RP Stmt {
        Node temp;        
        temp.subNode.push_back(createNode("FOR"));
...
ForExps
    : Def Exp SEMI Exp {
        ...
        temp.subNode.push_back(createNode("SEMI"));
        temp.subNode.push_back($4);
        temp.show = addLine("ForExps", @$.first_line);
        $$ = temp;
    }
    | SEMI Exp SEMI Exp {
        Node temp;        
